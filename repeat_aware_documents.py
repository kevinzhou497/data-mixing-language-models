...
def write_datafile(filename, tokens_np):
    """Writes a numpy array of tokens to a binary file with a robust 64-bit header."""
...
def create_shuffled_token_generator(dataset, permutation, encoder, eot_token):
    """
    A generator that yields tokenized documents in the exact order specified by the
    permutation array.
    """
    for doc_idx in permutation:
        ...
        # Skip examples with missing or empty abstract
        if abstract:
...
def process_and_write_split(split_name, dataset, indices, enc, eot, shard_size, output_dir):
    """
    Processes a given set of document indices and writes them to tokenized shards.
    Returns a tuple of (list_of_filenames, total_tokens_written).
    """
...
# ----------------- ARG PARSING -------------------
parser = argparse.ArgumentParser(description="Fast, non-streaming script with document-based subsampling and auto-splitting.")
...
# using this with pubmed so it retrieves data properly
...
if has_validation_split:
    ...
    # Create a single global shuffle
