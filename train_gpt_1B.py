...
parser.add_argument('--global_batch_size', type=int, default=128, help='Global batch size in sequences across all GPUs and accumulation steps')
parser.add_argument('--device_batch_size', type=int, default=8, help='Per-GPU microbatch size in sequences; lower this if you OOM')
parser.add_argument('--sequence_length', type=int, default=256, help='Sequence length in tokens')
parser.add_argument('--grad_checkpointing', action='store_true', help='Use activation checkpointing to reduce memory at the cost of speed')
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        for block in self.transformer.h:
            if self.training and getattr(self.config, "grad_checkpointing", False):
...
class Hyperparameters:
    ...
    # batch_size is GLOBAL across all GPUs and gradient accumulation steps.
    # device_batch_size is the per-GPU microbatch. Keep this small for large models.
...
microbatch_world = B * ddp_world_size
if args.batch_size < microbatch_world:
    raise ValueError(
        f"global_batch_size={args.batch_size} is smaller than device_batch_size*world_size="
        f"{B}*{ddp_world_size}={microbatch_world}. Lower --device_batch_size or raise --global_batch_size."
    )
if args.batch_size % microbatch_world != 0:
    raise ValueError(
        f"global_batch_size={args.batch_size} must be divisible by device_batch_size*world_size="
        f"{B}*{ddp_world_size}={microbatch_world}."
    )
# overall global batch size divided by (per-GPU microbatch * world size)
train_accumulation_steps = args.batch_size // microbatch_world
if master_process:
    print(f"global_batch_size={args.batch_size}, device_batch_size={B}, world_size={ddp_world_size}, grad_accum_steps={train_accumulation_steps}")
...
model = GPT(GPTConfig(vocab_size=num_vocab, n_layer=args.n_layer, n_head=args.n_head, n_embd=args.n_embd, grad_checkpointing=args.grad_checkpointing))
model = model.cuda()
